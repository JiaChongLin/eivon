...
export function WorkflowStudio() {
  ...
  function reference(step: Row, kind: "tool" | "model") {
    ...
    return <div className="step-grid"><label>{kind === "tool" ? "Tool" : "Model"}<select required value={ref?.id || ""} onChange={(e) => { const item = choices.find((item) => item.id === e.target.value); update(step.key, { [field]: item ? { id: item.id, version: item.active_version! } : undefined }); }}><option value="">Choose published {kind}</option>{ref && !choices.some((item) => item.id === ref.id) && <option value={ref.id}>{ref.id} (unavailable)</option>}{choices.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Version<input type="number" min={1} required value={ref?.version || ""} onChange={(e) => ref && update(step.key, { [field]: { ...ref, version: Number(e.target.value) } })} /></label></div>;
  }
  return <section>
    <div className="toolbar"><div><span className="eyebrow">ORCHESTRATION</span><h2 className="section-title">Workflow studio</h2></div><button className="button" disabled={busy} onClick={() => edit(null)}>+ New workflow</button></div>
    {error && <div className="notice error" role="alert">{error}</div>}{message && <div className="notice" role="status">{message}</div>}
    <div className="workflow-studio"><div className="workflow-list"><div className="chat-head">SAVED WORKFLOWS · {items.length}</div>{!items.length && <p className="muted">No workflows yet.</p>}{items.map((item) => <button disabled={busy} className={`workflow-item ${selected?.id === item.id ? "active" : ""}`} key={item.id} onClick={() => edit(item)}><strong>{item.name}</strong><small>{item.active_version ? `Published v${item.active_version}` : "Draft"}</small></button>)}</div>
      <form className="workflow-editor" onSubmit={(event) => { event.preventDefault(); void save(false); }}>
        <fieldset disabled={busy}><legend>{selected ? `Draft revision ${selected.revision}` : "New workflow"}</legend>
          <div className="step-grid"><label>Name<input required maxLength={160} value={name} onChange={(e) => setName(e.target.value)} /></label><label>Slug<input required disabled={!!selected} pattern="[a-z][a-z0-9_.-]{1,99}" value={slug} onChange={(e) => setSlug(e.target.value)} /></label></div>
